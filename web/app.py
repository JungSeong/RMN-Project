...
def predict(agentid):
    ...
    results = model.detect_emotion_for_single_frame(image)  # RMN 모델을 사용하여 이미지에서 감정 감지 수행
    print(results)

    image_with_bbx = model.draw(image, results)
    cv2.imwrite("./static"+image_path, image_with_bbx)

    # 가장 높은 점수를 받은 감정 결과 처리
...
def image_view():
    ...
    return render_template('uploadList.html', datas=listData) # render_template 메소드로 html 파일 불러오기
